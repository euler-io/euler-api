...
if [[ ${PROJECT_VERSION} == *"SNAPSHOT"* ]]; then
	snapshotVersion=$(curl -s https://oss.sonatype.org/content/repositories/snapshots/com/github/euler-io/opendistro-impl/${PROJECT_VERSION}/maven-metadata.xml | grep '<value>' | sort -r | head -1 | sed "s/.*<value>\([^<]*\)<\/value>.*/\1/")
