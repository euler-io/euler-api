...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        JWTAuthenticationFilter authenticationFilter = new JWTAuthenticationFilter(securityService, authenticationManager());
        authenticationFilter.setFilterProcessesUrl(LOGIN_PROCESSING_URL);
        JWTAuthorizationFilter authorizationFilter = new JWTAuthorizationFilter(securityService, authenticationManager());

        http.authorizeRequests().antMatchers(LOGIN_PROCESSING_URL, "/", "/swagger-ui.html").permitAll()
                .and().authorizeRequests()
                .antMatchers("/job/**", "/jobs/**", "/statistics/**")
