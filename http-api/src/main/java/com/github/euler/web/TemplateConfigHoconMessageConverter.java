...
public class TemplateConfigHoconMessageConverter extends AbstractGenericHttpMessageConverter<TemplateConfig> {
    ...
    @Override
    public TemplateConfig read(Type type, Class<?> contextClass, HttpInputMessage inputMessage) throws IOException, HttpMessageNotReadableException {
        return readInternal((Class<? extends TemplateConfig>) contextClass, inputMessage);
    }

    @Override
    protected TemplateConfig readInternal(Class<? extends TemplateConfig> clazz, HttpInputMessage inputMessage) throws IOException, HttpMessageNotReadableException {
    ...
    @Override
    protected void writeInternal(TemplateConfig t, Type type, HttpOutputMessage outputMessage) throws IOException, HttpMessageNotWritableException {
        Config config = (Config) t.getConfig();
        ConfigDocument document = ConfigDocumentFactory.parseString("{}")
                .withValueText("name", t.getName())
